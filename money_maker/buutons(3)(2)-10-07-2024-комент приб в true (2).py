...
# Декоратор для повторных попыток с задержкой в 10 секунд и ограничением в 3 попытки

def get_updates_with_retry():
    try:
        updates = bot.get_updates()
        for update in updates:
            bot.process_new_updates([update])
            # bot.send_message(message.chat.id, text=f"Произошла остановка бота запустил зново")
    except telebot.apihelper.ApiTelegramException as e:
        print(f"Failed to get updates: {e}")
        raise  # Передаем исключение дальше, чтобы активировать механизм повторных попыток


threading.Thread(target=check_bot, daemon=True).start()

# Декоратор для повторных попыток с задержкой в 10 секунд и ограничением в 3 попытки
# @retry(wait=wait_fixed(30), stop=stop_after_attempt(120))
# def get_updates_with_retry():
                ...
                while is_running:
                    ...
                    if delta < form['ints']['int_3']:
                        ...
                        if delta >= form['ints']['int_4']:
                            ...
                            bot.send_message(message.chat.id, f"delta={delta} и order5={order_5_is_active} ---> hold order1")
                            # Скворцов
                            if order_5_is_active:
                                try:
                                    if delta > form['ints']['int_1']:
...
#         try:
#             if CONSTANTS["old_message"]:
#                 bot.send_message(CONSTANTS["old_message"].chat.id,
#                                  text=f"Сбой TelegramAPI. Пробую запустить стратегию 1.")
#                 handle_stop(CONSTANTS["old_message"])
#                 bot.send_message(CONSTANTS["old_message"].chat.id,
#                                  text=f"STOP")
#                 handle_strategy_1(CONSTANTS["old_message"])
#         except Exception as e:
#             print(f'Ошибка во внутреннем try{e}')
#             time.sleep(30)
#     except Exception as e:
#         print(e)
#         time.sleep(15)
